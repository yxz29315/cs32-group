...
AstNode *Parser::ops(deque<Token> &x)
{
    ...
    while ((x.front().text == "(" || x.front().type == Token::TokenType::NUMBER || x.front().type == Token::TokenType::IDENTIFIER || x.front().type == Token::TokenType::OPERATOR) && (x.size() != (unsigned int)-1))
    {
        if (x.front().text == "(")
        {
            temp = SExpress(x);
